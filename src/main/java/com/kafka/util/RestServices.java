...
import com.kafka.model.KeyValueBean;

/**
 * A simple REST proxy that runs embedded in the {@link RestServices}.
 * This is used to demonstrate how a developer can use the Interactive Queries
 * APIs exposed by Kafka Streams to locate and query the State Stores within a
...
public class RestServices {
	...
	public RestServices(final KafkaStreams streams, final HostInfo hostInfo) {
	...
	public KeyValueBean byKey(@PathParam("storeName") final String storeName, @PathParam("key") final String key) {
	 ...
	 * @param storeName
	 *            store to query
	 * @return A List of {@link KeyValueBean}s representing all of the
	 *         key-values in the provided store
	 */
	@GET()
	 ...
	 * @return A List of {@link KeyValueBean}s representing all of the
	 *         key-values in the provided store that fall withing the given
	 *         range.
	 */
	@GET()
	...
	public List<KeyValueBean> keyRangeForStore(@PathParam("storeName") final String storeName,
			@PathParam("from") final String from, @PathParam("to") final String to) {
	 ...
	 *            key to look for
	 * @param from
	 *            time of earliest window to query
	 * @param to
	 *            time of latest window to query
	 * @return A List of {@link KeyValueBean}s representing the key-values for
	 *         the provided key across the provided window range.
	 */
	@GET()
	@Path("/windowed/{storeName}/{key}/{from}/{to}")
	@Produces(MediaType.APPLICATION_JSON)
	public List<KeyValueBean> windowedByKey(@PathParam("storeName") final String storeName,
			@PathParam("key") final String key, @PathParam("from") final Long from, @PathParam("to") final Long to) {

		// Lookup the WindowStore with the provided storeName
		final ReadOnlyWindowStore<String, Long> store = streams.store(storeName, QueryableStoreTypes.windowStore());
		if (store == null) {
			throw new NotFoundException();
		}
		...
		while (results.hasNext()) {
			...
			// convert the result to have the window time and the key (for
			// display purposes)
	...
	/**
	 * Get the metadata for all of the instances of this Kafka Streams
	 * application
	 * 
	...
	/**
	 * Performs a range query on a KeyValue Store and converts the results into
	 * a List of {@link KeyValueBean}
	 * 
	 * @param storeName
	 *            The store to query
	 * @param rangeFunction
	 *            The range query to run, i.e., all, from(start, end)
	 * @return List of {@link KeyValueBean}
	 */
	private List<KeyValueBean> rangeForKeyValueStore(final String storeName,
			final Function<ReadOnlyKeyValueStore<String, Long>, KeyValueIterator<String, Long>> rangeFunction) {
